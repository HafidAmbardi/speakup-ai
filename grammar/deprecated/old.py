    ...
    try:
        if request.data:
            ...
            for sentence in sentences:
                ...
                outputs = model_grammarly.generate(
                    input_ids,
                    max_length=256,
                    num_beams=4,
                    early_stopping=True
            ...
            try:
                progress_data = {
                    "userId": str(random.randint(1000, 9999)),  # Random 4-digit user ID
                    "activityName": "grammar exercise",
                    "activityDuration": 10,
                    "score": float(avg_distance),
                    "comment": f"{len(sentence_pairs)} total sentences"
                }
                
                progress_response = requests.post(
                    "https://pts.koyeb.app/api/progress/activityLog",
                    json=progress_data,
                    headers={"Content-Type": "application/json"}
                )
                
                if progress_response.status_code != 200:
                    logger.warning(f"Failed to send progress data: {progress_response.text}")
            except Exception as e:
                logger.error(f"Error sending progress data: {str(e)}")
